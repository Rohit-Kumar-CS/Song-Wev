...
// Define your songs data structure
const albumsData = {
    "ncs": {
        title: "NCS Songs",
        songs: [
            { name: "song1.mp3", title: "Song 1" },
            { name: "song2.mp3", title: "Song 2" }
            // Add more songs
        ]
    },
    "rock": {
        title: "Rock Songs",
        songs: [
            { name: "rock1.mp3", title: "Rock Song 1" },
            { name: "rock2.mp3", title: "Rock Song 2" }
            // Add more songs
        ]
    }
    // Add more albums
...
async function getSongs(folder) {
    try {
        currfolder = folder;
        const albumData = albumsData[folder.toLowerCase()];
        
        if (!albumData) {
            throw new Error(`Album ${folder} not found`);
        }

        // Create songs array with proper paths
        songs = albumData.songs.map(song => `/songs/${folder}/${song.name}`);

        const songUL = document.querySelector(".songlist ul");
        if (!songUL) return [];

        songUL.innerHTML = albumData.songs.map(song => `
            <li class="first">
                <div class="info">
                    <div>${song.title}</div>
                    <div>default</div>
        ...
        songUL.addEventListener("click", (e) => {
            ...
            const index = Array.from(songUL.children).indexOf(listItem);
            if (index !== -1) {
                playMusic(index);
            }
        });
...
async function displayAlbums() {
    try {
        ...
        cardContainer.innerHTML = Object.entries(albumsData).map(([folder, data]) => `
...
// The rest of your code (playMusic, formatTime, playNext, playPrevious) remains the same
